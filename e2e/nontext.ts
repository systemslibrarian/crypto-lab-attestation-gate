 ...
 * gives every interactive control a real edge — `.btn` and the tab buttons
 * take `--border-strong`, text inputs take it too, and the disclosure and
 * table wrappers take `--border` — but tokens are discarded by overrides, and
 * that is what this oracle exists to measure rather than trust. Three shapes
 * on this page override or bypass the base rule: `.btn-primary` and an active
 * `.tab-btn` repaint their border the SAME colour as their own accent fill,
 * so neither has an edge of its own and both live or die by fill-vs-surround;
 * `.btn[aria-pressed="true"]` does the same on every scenario selector; and
 * `.check[data-state="not-run"]` swaps to a dashed `--border-strong`. This
 * oracle judges each control as painted, at every driven state — including
 * the rejected-scenario and `aria-invalid` recolourings only the drive
 * reaches.
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * TPMS_ATTEST byte inspector: seventeen `.byte-field` rows, each with a
     * hex run and a decoded value, sitting beside a full `.checks` list and
     * four `.pcr-row` registers — all siblings re-walking the same ancestors
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // this page declares a transparent border today — every control paints
      // a real one — but the per-side test stays because the moment one does,
      // counting the spacer as a delineator would report a 1.00:1 edge that
      // nobody can see, which is a fabricated finding rather than a caught
      // one.
