 ...
 *    The three verdict tones (`.verdict[data-tone=ok|bad|alarm]`) are
 *    `color-mix(in oklab, tone ..%, var(--surface))`, and so are the passing
 *    and failing `.check` rows, the `.callout[data-tone=alarm]` boxes that
 *    carry every ground-truth statement, the `.stage` highlight and its
 *    divergent variant, the `.pcr-row[data-match=false]` rows, the
 *    `.byte-field[data-highlight]` fields in the TPMS_ATTEST inspector, the
 *    `.hex-old` / `.hex-new` halves of the PCR-extend preimage, the hero
 *    aside's accent wash, and `.btn-primary:hover`. The shared top bar adds a
 *    `color-mix(in srgb, ...)` for its ink. A violations-only gate therefore
 *    measured almost none of this lab's verdicts.
 *
 ...
 *    fractional opacity at all: muted text lowers the colour's LIGHTNESS
 *    (`--text-dim`, `--text-muted`) rather than fading it, and a disabled
 *    button changes its `color` rather than its alpha. The opacity handling
 *    below therefore has nothing to do here today, and stays because the first
 *    `opacity: .7` anyone adds will arrive without re-reading this file.
 *
 ...
 *    its long hex runs wrapping rather than scrolling, so the only real
 *    scrollers are the three `.table-wrap { overflow-x: auto }` regions — the
 *    PCR allocation table, the reset-authority table and the failure-code
 *    table. Content scrolled past a clipper is not dimmed or partly drawn; it
 *    is absent from the frame, and asking what colour it sits on has no
 *    answer. Its rect is outside every ancestor's box, so the walk would find
 *    nothing behind it and fall through to WHITE, inventing a failure for a
 *    row nobody can see.
 *
 ...
 * no author `content` at all today (the disclosure triangles on `<summary>`
 * are `::marker`, the UA's own, recoloured to the accent), so that half of
 * `nontext.ts` is inert here — and it runs at every state anyway, because the
 * first author-declared `content` on this page will arrive without anyone
 * re-reading this file.
 *
 * SECOND, `aria-hidden` text that is still painted — see the `ariaHidden` note
 * below. This page hides only glyphs (checks, crosses, dashes, the exclamation
 * on an alarm verdict, and the tab numbers) that sit beside their own words,
 * but they are painted in the semantic inks on the `color-mix()` tints, so
 * `gate.ts` measures every `aria-hidden` subtree explicitly with that
 ...
 * and check-row glyphs and the ordinal inside each tab button, each painted in
 * a semantic ink (`--ok-text`, `--bad-text`, `--alarm-text`) on its own
 * `color-mix()` tint — the states this lab exists to show — so `scan()` calls
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     * the TPMS_ATTEST byte inspector: seventeen `.byte-field` rows, each with
     * its own hex run, decoded value and teaching note, sitting beside four
     * `.pcr-row` registers of 64-character hex and a full `.checks` list —
     * all of them siblings re-walking the same ancestors up to `<body>`.
     ...
     * that is the three `.table-wrap { overflow-x: auto }` regions — this page
     * wraps its long hex instead of scrolling it, so the tables are the only
     * scrollers — but the collection is built from the live DOM, so a new
     * scroller joins it without an edit here.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // of the shape: every `details.disclosure` ships shut, and that is where
      // the byte inspector, the CBOR, the reset-authority table and the
      // specification quotations all live. The gate opens them by clicking
      // their <summary>, which is the route a reader has, rather than setting
      // `.open` from script — which is what the gate this replaces did, to
      // every <details> on the page, before its only scan.
     ...
     * UI helpers: the `.verdict-icon` on each verdict, the `.check-icon` on
     * each check row, and the `.tab-num` ordinal inside each tab button —
     * plus the shared header's two SVG marks, which carry no text. Each glyph
     * duplicates the words directly beside it (the check rows read "PASS —"
     * and "FAIL —" in text, the verdicts spell out ATTESTED or REJECTED), but
     * each is painted in a SEMANTIC ink (`--ok-text`, `--bad-text`,
     ...
     *
     * Nothing on this page hides a VALUE. No digest, register, nonce,
     * signature, failure code or verdict text is inside an `aria-hidden`
